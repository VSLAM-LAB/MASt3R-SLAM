...
from mast3r_slam.frame import Mode, SharedKeyframes, SharedStates, create_frame, Pose
...
if __name__ == "__main__":
    ...
    if not no_viz:
        viz = mp.Process(
            ...
            args=(states, keyframes, main2viz, viz2main, args.settings_yaml),
        )
        viz.start()
    ...
    backend = mp.Process(
        target=run_backend, args=(args.settings_yaml, model, states, keyframes, K)
    )
    backend.start()
    ...
    #eval.save_ATE(args.exp_folder, 
                  #args.exp_it.zfill(5) + '_KeyFrameTrajectory' + '.txt', dataset.timestamps, poses)
    eval.save_ATE(args.exp_folder, 
